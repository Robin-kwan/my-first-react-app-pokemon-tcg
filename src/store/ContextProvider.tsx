...
function ContextProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (name.length > 0) {
      totalFilteredItems = totalFilteredItems.filter((item) =>
        item.name.toLowerCase().includes(name)
      );
    }
    // filter by set name
    if (set) {
      totalFilteredItems = totalFilteredItems.filter(
        (item) => item.set.name === set
      );
    }
    // filter by rarity
    if (rarity) {
    ...
    PokemonTCG.getAllCards()
      .then((result: PokemonTCG.Card[]) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // get rarity list
          if (
            !rarityData.includes(data[i].rarity) &&
            data[i].rarity !== undefined
          ) {
  ...
  function getCards(items: PokemonTCG.Card[]) {
    const data = [];
    for (
      let i = itemPerPage * currentPage - itemPerPage;
      i < itemPerPage * currentPage;
      i++
    ) {
      if (items[i]) {
        data.push(items[i]);
